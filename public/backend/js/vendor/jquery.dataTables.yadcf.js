...
(function (root, factory) {
	...
	var yadcf = (function () {
		...
		function addRangeDateFilterCapability(table_selector_jq_friendly, fromId, toId, col_num, date_format) {
			...
			$.fn.dataTableExt.afnFiltering.push(
				function (settingsDt, aData, iDataIndex, rowData) {
					...
					try {
						if (min.length === (date_format.length + 2) || columnObj.datepicker_type === 'bootstrap-datetimepicker' || columnObj.datepicker_type === 'rome') {
							if (columnObj.datepicker_type === 'jquery-ui') {
								min = (min !== "") ? $.datepicker.parseDate(date_format, min) : min;
							} else if (columnObj.datepicker_type === 'bootstrap-datetimepicker' || columnObj.datepicker_type === 'rome') {
								min = (min !== "") ? moment(min, columnObj.moment_date_format).toDate() : min;
							}
						}
					} catch (err1) {}
					try {
						if (max.length === (date_format.length + 2) || columnObj.datepicker_type === 'bootstrap-datetimepicker' || columnObj.datepicker_type === 'rome') {
							if (columnObj.datepicker_type === 'jquery-ui') {
		...
		function addRangeDateFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, date_format) {
			...
			$(filter_selector_string).append("<span class=\"yadcf-filter-range-date-seperator\" > &mdash; </span>");
			...
			} else if (columnObj.datepicker_type === 'rome') {
                datepickerObj.inputFormat = date_format;
            }
			...
			if (columnObj.datepicker_type === 'jquery-ui') {
                ...
                var dateFromInput = document.querySelector("#" + fromId);
                var dateToInput = document.querySelector("#" + toId);

                /*
                rome(dateFromInput, $.extend(datepickerObj, {
                    dateValidator: rome.val.beforeEq(dateToInput)
                }));
                rome(dateToInput, $.extend(datepickerObj, {
                    dateValidator: rome.val.afterEq(dateFromInput)
                }));
                */

                rome(dateFromInput, datepickerObj);
                if (columnObj.externally_triggered !== true) {
                    rome(dateFromInput).on('data', function (value) {
                        dateSelect(value, dateFromInput);
                    });
                }
                rome(dateToInput, datepickerObj);
                if (columnObj.externally_triggered !== true) {
                    rome(dateToInput).on('data', function (value) {
                        dateSelect(value, dateToInput);
                    });
                }
            }
		...
		function addDateFilter(filter_selector_string, table_selector_jq_friendly, column_number, filter_reset_button_text, filter_default_label, date_format) {
			...
			if (columnObj.datepicker_type === 'jquery-ui') {
                ...
                rome(dateInput, datepickerObj);
                rome(dateInput).on('data', function (value) {
                    dateSelectSingle(value, dateInput);
                });
            }
					...
					case 'date':
						switch (optionsObj.select_type) {
    						...
    						case 'bootstrap-datetimepicker':
    							...
    							break;
                            case 'rome':
                                var picker = rome(document.querySelector("#yadcf-filter-" + table_selector_jq_friendly + "-" + column_number));
                                picker.destroy();
                                break;
						}
						break;
					case 'range_date':
						...
						switch (optionsObj.select_type) {
    						...
    						case 'bootstrap-datetimepicker':
    							...
    							break;
                            case 'rome':
                                var fromPicker = rome(document.querySelector("#" + fromId));
                                fromPicker.destroy();
                                var toPicker = rome(document.querySelector("#" + toId));
                                toPicker.destroy();
                                break;
						}
						break;
		...
		function appendFilters(oTable, args, table_selector, pSettings) {
			...
			for (columnObjKey in args) {
				if (args.hasOwnProperty(columnObjKey)) {
                    ...
                    //for rome
                    if (columnObj.datepicker_type === 'rome' && columnObj.filter_plugin_options !== undefined && columnObj.filter_plugin_options.inputFormat !== undefined) {
                        date_format = columnObj.filter_plugin_options.inputFormat;
                    }
