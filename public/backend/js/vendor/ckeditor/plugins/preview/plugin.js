	...
	var previewCmd = { modes: { wysiwyg: 1, source: 1 },
		...
		exec: function( editor ) {
			...
			if ( config.fullPage )
				...
				var bodyHtml = '<body style="padding: 0px;" ',
				...
				sHTML = editor.config.docType + '<html dir="' + editor.config.contentsLangDirection + '">' +
					'<head>' +
    					...
    					'<script src="../../../frontend/assets/js/vendor/modernizr.min.js"></script>' +
					'</head>' + bodyHtml +
					'<main class="site-main" style="padding: 0px; margin: 0px;">' +
				        '<div class="site-main__wrapper" style="padding: 12px 24px 12px 12px; margin: 0px;">' +
				        editor.getData() +
				        '</div>' +
				    '</main>' +
					'<script>window.myPrefix = "../../../frontend/assets/";</script>' +
