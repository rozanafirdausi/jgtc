...
<div class="page-bar">
    <ul class="page-breadcrumb">
        ...
        <li>
            <a href="{{ route('backend.product.show', ['id'=>$baseObject->product_id]) }}">{{ ($baseObject->product ? $baseObject->product->getFormattedValue() : "Unknown Product") }}</a>
            <i class="fa fa-circle"></i>
        </li>
        <li>
            <span>Add New {{ $baseObject->_label }}</span>
        </li>
    </ul>
</div>
<!-- END PAGE BAR -->
...
<!-- BEGIN PAGE TITLE-->
<h3 class="page-title">Add New {{ $baseObject->_label }} To {{ ($baseObject->product ? $baseObject->product->getFormattedValue() : "Unknown Product") }}
