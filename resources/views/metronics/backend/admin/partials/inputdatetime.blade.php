...
<div class="form-group form-md-line-input {{ $formSetting['errors'] ? 'has-error' : '' }}" id="{{ $formSetting['container_id'] }}">
    ...
    <div class="col-md-10">
        <input data-datetime-input class='form-control' id='{{ $formSetting['id'] }}' type='text' name="{{ $formSetting['extended_model'] ?  'extended[' . $formSetting['name'] . ']' : $formSetting['name'] }}" value='{{ $formSetting['value'] }}' {{ $formSetting['required'] ? 'required' : '' }}>
