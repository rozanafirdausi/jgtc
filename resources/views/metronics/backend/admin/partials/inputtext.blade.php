...
<div class="form-group form-md-line-input {{ $formSetting['errors'] ? 'has-error' : '' }}" id="{{ $formSetting['container_id'] }}">
    ...
    <div class="col-md-10">
        ...
        <div class="input-icon {!! isset($formSetting['group']['icon-position']) ? $formSetting['group']['icon-position'] : '' !!}">
            <input type="text" class="form-control" id="{{ $formSetting['id'] }}" placeholder="enter {{ strtolower($formSetting['label']) }}" name="{{ $formSetting['extended_model'] ?  'extended[' . $formSetting['name'] . ']' : $formSetting['name'] }}" value='{{ $formSetting['value'] }}' {{ $formSetting['required'] ? 'required' : '' }} {{ $formSetting['readonly'] ? 'readonly' : '' }}>
