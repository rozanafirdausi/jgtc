...
<div class='form-row' id='{{ $formSetting['container_id'] }}'>
    <div class='bzg'>
        ...
        <div class='bzg_c' data-col='l8'>
            <textarea rows='5' data-autosize class='form-input' id='{{ $formSetting['id'] }}' name='{{ $formSetting['name'] }}' {{ $formSetting['required'] ? 'required' : '' }} {{ $formSetting['readonly'] ? 'readonly' : '' }}>{{ $formSetting['value'] }}</textarea>
