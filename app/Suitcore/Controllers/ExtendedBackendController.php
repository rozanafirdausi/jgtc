...
use Suitcore\Repositories\ExtendedSuitRepository;
 ...
 **/
class ExtendedBackendController extends Controller
{
    /**
     ...
     * @return void
     */
    public function __construct(ExtendedSuitRepository $_baseRepo, $_baseModel, $_topLevelRelationModelString = null)
    {
        // set property
    ...
    public function getIndex()
    {
        ...
        return view($this->viewBaseClosure . '.index')->with($this->viewInstanceName, $this->baseModel)
                                                      ->with($this->viewExtendedInstanceName, $this->baseModel->getRelatedObject('extended'))
    ...
    public function postIndexJson() {
        ...
        $extendedObj = $baseObj->getRelatedObject('extended');
    ...
    public function getView($id)
    {
        ...
        //DD($fetchedData['extendedObject']->toArray());
        //dd($fetchedData['object']->toArray());
        $extendedObjectData = $fetchedData['extendedObject'];
        if ($extendedObjectData == null) {
            $extendedObjectData = $baseObj->getRelatedObject('extended');
        }
        // Return
        return view($this->viewBaseClosure . '.show')
             ->with($this->viewInstanceName, $fetchedData['object'])
             ->with($this->viewExtendedInstanceName, $extendedObjectData);
    }
    ...
    public function getCreate()
    {
        ...
        return view($this->viewBaseClosure . '.create')
             ->with($this->viewInstanceName, $baseObj)
             ->with($this->viewExtendedInstanceName, $extendedObj);
    }
    ...
    public function postCreate() {
        ...
        $baseObj = $this->baseModel;
        $extendedObj = $baseObj->getRelatedObject('extended');
        $result = $this->baseRepository->create($param, $baseObj, $extendedObj);
        if ($baseObj->uploadError || $extendedObj->uploadError) {
        ...
        $firstError = null;
        if ($allError = $baseObj->errors) {
            $firstError = $baseObj->errors->first();
        } elseif ($allError = $extendedObj->errors) {
            $firstError = $extendedObj->errors->first();
        }
        //dd($allError, $param);
        $this->showNotification(self::ERROR_NOTIFICATION, $baseObj->_label . ' Not Created', $firstError);
        return Redirect::route($this->routeBaseName . '.create')->with('errors', $allError)->withInput($param);
    }
    ...
    public function getUpdate($id) {
        ...
        $extendedObjectData = $baseObj->extended;
        if ($extendedObjectData == null) {
            $extendedObjectData = $baseObj->getRelatedObject('extended');
        }

        return view($this->viewBaseClosure . '.edit')
             ->with($this->viewInstanceName, $baseObj)
             ->with($this->viewExtendedInstanceName, $extendedObjectData);
    }
    ...
    public function postUpdate($id) {
        ...
        $baseObj = $this->baseModel;
        $extendedObj = $baseObj->getRelatedObject('extended');
        $result = $this->baseRepository->update($id, $param, $baseObj, $extendedObj);
        if ($baseObj->uploadError || $extendedObj->uploadError) {
        ...
        $this->showNotification(self::ERROR_NOTIFICATION, $baseObj->_label . ' Not Updated', $firstError);
        return Redirect::route($this->routeBaseName . '.update', ['id'=>$id])->with('errors', $allError)->withInput($param);
    }
    ...
    public function exportToExcel($specificFilter = null)
    {
        if ($this->baseModel) {
            $baseObj = $this->baseModel;
            $extendedBaseObject = $baseObj->getRelatedObject('extended');
            // BOX SPOUT WAY
            ...
            $callback = function () use ($baseObj, $extendedBaseObject, $specificFilter) {
                ...
                foreach ($extendedBaseObject->getBufferedAttributeSettings() as $key=>$val) {
                    if ( $val['visible'] ) {
                        $columnHeader[] = $val['label'];
                    }
                }
                $writer->addRow($columnHeader); // Header - add a row at a time
                $objectList = $baseObj->with('extended')->select($baseObj->getTable().".*");
                ...
                $objectList->chunk(100, function($objects) use ($writer, $extendedBaseObject, $specificFilter) {
                    // init data
                    $data = [];
                    foreach ($objects as $object) {
                        ...
                        foreach ($extendedBaseObject->getBufferedAttributeSettings() as $attrName=>$attrSettings) {
                            if ($attrSettings['visible']) {
                                if ($specificFilter == null ||
                                    !is_array($specificFilter) ||
                                    !isset($specificFilter[$attrName])) {
                                    $tmpRow[] = $object->extended ? $object->extended->$attrName : '-';
                                }
                            }
                        }
                        $data[] = $tmpRow;
