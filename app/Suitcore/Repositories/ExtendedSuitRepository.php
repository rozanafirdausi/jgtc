...
use Suitcore\Repositories\Contract\ExtendedSuitRepositoryContract;
use Suitcore\Datatable\DatatableTrait;

class ExtendedSuitRepository implements ExtendedSuitRepositoryContract
{
    use DatatableTrait;
    ...
    public function jsonDatatable($param, $columnFormatted = null, $specificFilter = null, $optionalFilter = null, $columnException = null) {
        ...
        $extendedTmpObject = $tmpObject->getRelatedObject('extended');
        $tmpObject->showAllOptions = true;
        $object = $tmpObject->with('extended')->select($tmpObject->getTable().".*");
        $datatableSelection = [];
        ...
        foreach($jsonSource['rawdata'] as $obj) {
            ...
            foreach ($extendedTmpObject->getBufferedAttributeSettings() as $attrName=>$attrSettings) {
                if (isset($attrSettings['visible']) &&
                    $attrSettings['visible'] &&
                    ($specificFilter == null ||
                     !is_array($specificFilter) ||
                    ...
                    ($columnException == null ||
                     !is_array($columnException) ||
                     !in_array($attrName, $columnException))) {
                    $tmpRow[] = $obj->extended ? $obj->extended->$attrName : '-';
                }
            }
    ...
    public function get($objectId)
    {
        ...
        $extendedObject = ($object->extended ? $object->extended : null);
        return [
            'object' => $object,
    ...
    /**
     * Main function for create model and its extended model
     * @param  array          $param           value of main model and extended model
     * @param  SuitModel|null &$object         Main model object
     * @param  SuitModel|null &$extendedObject Extended object
     * @return SuitModel|false
     */
    public function create($param, SuitModel &$object = null, SuitModel &$extendedObject = null) {
        DB::beginTransaction();
        // Create main model
        $object = ($this->mainModel ? $this->mainModel->getNew() : new SuitModel);
        $result = $this->createModel($param, $object);
        if ($result) {
            // Check for extended model
            if ($extendedObject) {
                $mainObject = ($this->mainModel ? $this->mainModel->find($result->id) : SuitModel::find($id));
                $mainObjectForeignId = $mainObject->getExtendedForeignKey();
                $param = $param['extended'];
                $param[$mainObjectForeignId] = $result->id;
                $extendedResult = $this->createModel($param, $extendedObject);
                if ($extendedResult) {
                    DB::commit();
                } else {
                    DB::rollback();
    ...
    public function createModel($param, SuitModel &$object = null) {
    ...
    /**
     * Main function for update model and its extended model
     * @param  integer        $id              Main model ID
     * @param  array          $param           value of main model and extended model
     * @param  SuitModel|null &$object         Main model object
     * @param  SuitModel|null &$extendedObject Extended object
     * @return SuitModel|false
     */
    public function update($id, $param, SuitModel &$object = null, SuitModel &$extendedObject = null) {
        DB::beginTransaction();
        // Update main model
        $object = ($this->mainModel ? $this->mainModel->find($id) : SuitModel::find($id));
        $result = $this->updateModel($id, $param, $object);
        if ($result) {
            // Check for extended model
            if ($extendedObject) {
                $mainObject = ($this->mainModel ? $this->mainModel->find($id) : SuitModel::find($id));
                $param = $param['extended'];
                $extendedObject = ($mainObject->extended ? $mainObject->extended : null);
                if ($extendedObject == null) {
                    $extendedObject = $this->mainModel->getRelatedObject('extended');
                    $mainObjectForeignId = $mainObject->getExtendedForeignKey();
                    $param[$mainObjectForeignId] = $result->id;
                    $extendedResult = $this->createModel($param, $extendedObject);
                } else {
                    $extendedResult = $this->updateModel($id, $param, $extendedObject);
                }
                if ($extendedResult) {
                    DB::commit();
                } else {
    ...
    public function updateModel($id, $param, SuitModel &$object = null)
    {
